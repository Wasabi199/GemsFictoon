    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            ...
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
             ...
             <!-- Email Address -->
             <div class="mt-4">
                <x-input-label for="email" :value="__('User Type')" />
                <select name="usertype" class="w-full text-white border-gray-700 rounded-md bg-slate-900">
                    <option disabled selected class="py-10 text-white">Select Usertype</option>
                    @foreach ($types as $type)
                    <option value="{{$type['value']}}" class="py-10 text-white">{{$type['title'] }}</option>
                    @endforeach
                </select>
